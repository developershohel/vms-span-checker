 ...
 * `VEFG_REGISTRATION_GUARD_LOG` as true to log hook/outcome lines without full WP_DEBUG.
 *
 ...
 * To have {@see wp_insert_user()} return {@see WP_Error} with code `vefg_registration_email` instead of
...
class Registration_Guard {
	...
	public const OPTION_KEY = 'vefg-registration-guard';

	private const RATE_TRANSIENT_PREFIX = 'vefg_rg_rl_';
	...
	public function __construct() {
		...
		// Validation
		add_filter( 'registration_errors', array( $this, 'filter_registration_errors' ), -9999, 3 );

		// Pre-insert
		add_filter( 'wp_pre_insert_user_data', array( $this, 'filter_wp_pre_insert_user_data' ), PHP_INT_MIN, 4 );
		add_filter( 'wp_pre_insert_user_data', array( $this, 'filter_wp_pre_insert_user_data_return_wp_error' ), PHP_INT_MAX, 4 );

		// SQL guard
		add_filter( 'query', array( $this, 'filter_wpdb_query_block_raw_user_insert' ), -9999, 1 );

		// Post insert
		add_action( 'user_register', array( $this, 'action_user_register_enforce_guard' ), -9999, 2 );

		// Optional
		add_filter( 'gettext', array( $this, 'filter_gettext_empty_data_replacement' ), -9999, 3 );
		add_action( 'template_redirect', array( $this, 'consume_registration_error_query' ), 5 );
		add_action( 'login_init', array( $this, 'consume_registration_error_query' ), 5 );
		add_action( 'wp_footer', array( $this, 'maybe_print_registration_notice_fallback' ), 6 );
		add_action( 'login_form_register', array( $this, 'print_registration_notice_login_form' ), 5 );
		add_action( 'wp_enqueue_scripts', array( $this, 'enqueue_frontend_registration_blocked_assets' ), 26 );
		add_action( 'login_enqueue_scripts', array( $this, 'enqueue_frontend_registration_blocked_assets' ), 26 );
	...
	private function apply_guard( $errors, string $user_email, string $hook_source ) {
		...
		if ( $msg !== null ) {
			$errors->add( 'vefg_registration_email', $msg );
		}
	...
	private static function get_block_message_for_email( string $user_email, string $hook_source = 'unknown' ): ?string {
		...
		if ( null === $msg ) {
			...
			if ( function_exists( 'vms_elements_form_guard_record_strike' ) ) {
				vms_elements_form_guard_record_strike( (string) $result['message'], 'registration', 0, $user_email );
			}
	...
	private static function append_attempt_context( array $cfg, string $ip ): string {
		...
		return vms_elements_form_guard_get_error_message( 'reg_rate_limit_count', array( $current, $max ) );
	}
	...
	private static function registration_failed_check_line( string $reason ): string {
		...
		if ( false !== strpos( $r, 'does not resolve in dns' ) ) {
			return vms_elements_form_guard_get_error_message( 'reg_dns_failed' );
		}
		if ( false !== strpos( $r, 'no mx' ) ) {
			return vms_elements_form_guard_get_error_message( 'reg_mx_failed' );
		}
		if ( false !== strpos( $r, 'disposable' ) ) {
			return vms_elements_form_guard_get_error_message( 'reg_disposable' );
		}
		if ( false !== strpos( $r, 'too many failed registration attempts' )
			|| false !== strpos( $r, 'daily registration attempt limit reached' ) ) {
			return vms_elements_form_guard_get_error_message( 'reg_rate_limit' );
		}
		return vms_elements_form_guard_get_error_message( 'reg_reputation_failed' );
	}
	...
	private static function brand_registration_block_message( string $reason, string $attempt_line = '' ): string {
		$parts   = array();
		$parts[] = vms_elements_form_guard_get_error_message( 'reg_blocked_intro' );
		$parts[] = self::registration_failed_check_line( $reason );
		if ( '' !== $attempt_line ) {
			$parts[] = $attempt_line;
		}
		$parts[] = vms_elements_form_guard_get_error_message( 'reg_contact_admin' );
	...
	private static function maybe_redirect_registration_error( string $message, string $hook_source ): void {
		...
		set_transient(
			'vefg_rerr_' . $token,
			array(
				'title'   => vms_elements_form_guard_get_error_message( 'reg_blocked_title' ),
				'message' => $message,
				'hook'    => $hook_source,
			),
			5 * MINUTE_IN_SECONDS
		...
		wp_safe_redirect( add_query_arg( 'vefg_reg_err', $token, $target ) );
		exit;
	}
	...
	public function consume_registration_error_query(): void {
		...
		if ( is_admin() || empty( $_GET['vefg_reg_err'] ) ) {
			return;
		}
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Validated against server-stored transient below.
		$token = sanitize_text_field( wp_unslash( (string) $_GET['vefg_reg_err'] ) );
		if ( strlen( $token ) !== 16 || ! ctype_alnum( $token ) ) {
			return;
		}
		$data = get_transient( 'vefg_rerr_' . $token );
		delete_transient( 'vefg_rerr_' . $token );
		if ( ! is_array( $data ) || empty( $data['message'] ) ) {
			return;
		}
		self::$frontend_registration_notice = array(
			'title'   => isset( $data['title'] ) ? (string) $data['title'] : vms_elements_form_guard_get_error_message( 'reg_blocked_title' ),
	...
	public function maybe_print_registration_notice_fallback(): void {
		...
		echo '<div class="vefg-registration-blocked-notice" role="alert" style="max-width:720px;margin:1rem auto;border-left:4px solid #d63638;background:#fcf0f1;padding:12px 14px;border-radius:4px;color:#1d2327;font-size:14px;line-height:1.45;">';
	...
	public function enqueue_frontend_registration_blocked_assets(): void {
		...
		wp_add_inline_script(
			...
			. 'if(window.history&&history.replaceState){var u=new URL(window.location.href);u.searchParams.delete("vefg_reg_err");'
	...
	/**
	 * Writes one JSON line to PHP’s error log when {@see WP_DEBUG_LOG} or `VEFG_REGISTRATION_GUARD_LOG` is enabled.
	 *
	...
	private static function log_registration_guard( string $hook, string $outcome, array $context = array() ): void {
		...
		if ( self::is_registration_guard_file_log_enabled() ) {
			...
			error_log( '[VMS Elements Form Guard] ' . wp_json_encode( $row, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE ) );
		}
	}

	private static function is_registration_guard_file_log_enabled(): bool {
		if ( defined( 'VEFG_REGISTRATION_GUARD_LOG' ) ) {
			return (bool) VEFG_REGISTRATION_GUARD_LOG;
		}
	...
	private static function rate_limit_message_if_blocked( array $cfg, string $ip ): ?string {
		...
		if ( ! empty( $state['locked_until'] ) && $now < (int) $state['locked_until'] ) {
			...
			return sprintf(
				...
				__( 'Too many failed registration attempts from your network. Please try again in about %1$s. Reference: %2$s', 'vms-elements-form-guard' ),
				$eta,
				$ref
			);
		}
		...
		if (
			...
			return sprintf(
				...
				__( 'Daily registration attempt limit reached for your network. Please try again tomorrow. Reference: %s', 'vms-elements-form-guard' ),
				$ref
			);
		}
	...
	private static function rate_reference_id( string $ip ): string {
		...
		return strtoupper( substr( sha1( $ip . '|' . $day . '|vefg_rg' ), 0, 10 ) );
	}
	...
	private static function append_rate_reference_id( string $message, string $ip ): string {
		...
		return $message . ' ' . sprintf(
			/* translators: %s: support reference id */
			__( '(Reference: %s)', 'vms-elements-form-guard' ),
			$ref
		);
	}
	...
	public function filter_wp_pre_insert_user_data( $data, $update, $user_id, $userdata ) {
		...
		if ( $msg !== null ) {
			self::$wp_pre_insert_user_error       = new WP_Error( 'vefg_registration_email', $msg );
