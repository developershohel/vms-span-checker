...
jQuery(function ($) {
                ...
                validateUrlServer(domainName, 'email', apiSettings)
                    .then(function (result) {
                        if (result.status) {
                            ...
                            wpSpanCheckerToast.fire({
                                icon: 'error',
                                title: result.message || 'Email address is invalid',
                            });
                            disableSubmitButton(submitButton);
                        }
                    })
    ...
    settings.forEach(function (setting) {
        ...
        if (
            page_id &&
            page_id !== 'all-pages' &&
            String(page_id) !== String(currentPageID)
        ) {
            return;
        }
        ...
        if (!$form.length) {
            wpSpanCheckerToast.fire({
                icon: 'error',
                title:
                    'Form not found. Check Form ID / class in WP Spam Checker settings.',
            });
            return;
        }
    ...
    if ($verifyTempEmailBtn.length && $email.length && $password.length) {
        $verifyTempEmailBtn.on('click', function (e) {
            ...
            if (getEmail === '') {
                wpSpanCheckerToast.fire({ icon: 'error', title: 'Email is required' });
                return;
            }
            if (password === '') {
                wpSpanCheckerToast.fire({ icon: 'error', title: 'Password is required' });
                return;
            }
            ...
            validateUrlServer(regDomain, 'registration', [
                ...
                .then(function (res) {
                    if (res.status) {
                        ...
                        wpSpanCheckerToast.fire({
                            icon: 'error',
                            title: res.message || 'Email or domain is not valid',
                        });
                    }
                })
                .catch(function (err) {
