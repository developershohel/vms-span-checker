...
if ( isset( $_SERVER['REQUEST_METHOD'] ) && 'POST' === $_SERVER['REQUEST_METHOD'] ) {
	...
	if ( isset( $_POST['google_config'] ) ) {
		...
		update_option( 'wsc-google-config', $google_config );
		echo '<div class="updated"><p>' . esc_html__( 'Google configuration saved.', 'vms-span-checker' ) . '</p></div>';
	}
	...
	if ( ! empty( $_POST['virustotal_key'] ) ) {
		...
		if ( ! in_array( $new_key, $virustotal_config['keys'], true ) ) {
			...
			echo '<div class="updated"><p>' . esc_html__( 'VirusTotal API key added.', 'vms-span-checker' ) . '</p></div>';
		}
	}
	...
	if ( ! empty( $_POST['delete_vt_key'] ) ) {
		...
		update_option( 'wsc-virustotal-config', $virustotal_config );
		echo '<div class="updated"><p>' . esc_html__( 'VirusTotal API key removed.', 'vms-span-checker' ) . '</p></div>';
	}
	...
	if ( ! empty( $_POST['virustotal_thresholds_save'] ) ) {
		...
		update_option( 'wsc-virustotal-config', $virustotal_config );
		echo '<div class="updated"><p>' . esc_html__( 'VirusTotal thresholds saved.', 'vms-span-checker' ) . '</p></div>';
	}
	...
	if ( isset( $_POST['recaptcha_save'] ) ) {
		...
		echo '<div class="updated"><p>' . esc_html__( 'Google reCAPTCHA configuration saved.', 'vms-span-checker' ) . '</p></div>';
	}
}
...
<div class="wrap wsc-admin">
	...
	vms_span_checker_admin_page_header(
		...
		__( 'Connect Google Web Risk, VirusTotal, and reCAPTCHA for enhanced form protection.', 'vms-span-checker' )
	);
	?>

	<div class="wsc-card wsc-api-section">
		<h2 class="wsc-card__title"><?php esc_html_e( 'Google reCAPTCHA', 'vms-span-checker' ); ?></h2>
		<p class="description" style="margin-top:0;">
			<?php esc_html_e( 'Add Google reCAPTCHA v2 (checkbox) or v3 (invisible) to protect your forms from bots and spam.', 'vms-span-checker' ); ?>
		</p>
		<form method="post">
			...
			<table class="form-table" role="presentation">
				<tr>
					<th scope="row"><label for="recaptcha_version"><?php esc_html_e( 'reCAPTCHA Version', 'vms-span-checker' ); ?></label></th>
					<td>
						<select name="recaptcha_version" id="recaptcha_version" class="regular-text">
							<option value="v2" <?php selected( $recaptcha_config['version'], 'v2' ); ?>><?php esc_html_e( 'reCAPTCHA v2 (Checkbox)', 'vms-span-checker' ); ?></option>
							<option value="v3" <?php selected( $recaptcha_config['version'], 'v3' ); ?>><?php esc_html_e( 'reCAPTCHA v3 (Invisible)', 'vms-span-checker' ); ?></option>
						</select>
						<p class="description">
							<?php esc_html_e( 'v2: Shows "I\'m not a robot" checkbox. v3: Invisible, scores user behavior.', 'vms-span-checker' ); ?>
						</p>
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="recaptcha_site_key"><?php esc_html_e( 'Site Key', 'vms-span-checker' ); ?></label></th>
					<td>
				...
				<tr>
					<th scope="row"><label for="recaptcha_secret_key"><?php esc_html_e( 'Secret Key', 'vms-span-checker' ); ?></label></th>
					<td>
						...
						<p class="description">
							...
							echo wp_kses(
								sprintf(
									...
									__( 'Get your keys from <a href="%s" rel="noopener noreferrer" target="_blank">Google reCAPTCHA Admin Console</a>.', 'vms-span-checker' ),
			...
			<p class="submit">
				<input type="submit" class="button button-primary" value="<?php esc_attr_e( 'Save reCAPTCHA configuration', 'vms-span-checker' ); ?>">
			</p>
		</form>
	</div>

	<div class="wsc-card wsc-api-section">
		<h2 class="wsc-card__title"><?php esc_html_e( 'Google Web Risk', 'vms-span-checker' ); ?></h2>
		<p class="description" style="margin-top:0;">
			<?php
			echo esc_html__(
		...
		<form method="post">
			...
			<table class="form-table" role="presentation">
				<tr>
					<th scope="row"><label for="api_key"><?php esc_html_e( 'API key', 'vms-span-checker' ); ?></label></th>
					<td>
						...
						<p class="description">
							...
							echo wp_kses(
								sprintf(
									...
									__( 'See <a href="%s" rel="noopener noreferrer" target="_blank">Google Web Risk documentation</a> to enable the API and create a key.', 'vms-span-checker' ),
			...
			<p class="submit">
				<input type="submit" class="button button-primary" value="<?php esc_attr_e( 'Save Google configuration', 'vms-span-checker' ); ?>">
			</p>
		</form>
	</div>

	<div class="wsc-card wsc-api-section">
		<h2 class="wsc-card__title"><?php esc_html_e( 'VirusTotal API keys', 'vms-span-checker' ); ?></h2>
		<form method="post" class="wsc-card" style="margin-bottom:1.5em;">
			<?php wp_nonce_field( 'wsc_api_action', 'wsc_api_nonce' ); ?>
			<input type="hidden" name="virustotal_thresholds_save" value="1">
			<table class="form-table" role="presentation">
				<tr>
					<th scope="row"><label for="vt_max_malicious"><?php esc_html_e( 'Max malicious engines', 'vms-span-checker' ); ?></label></th>
					<td>
						...
						<p class="description"><?php esc_html_e( 'Reject the domain if VirusTotal reports more than this many “malicious” detections. Use 0 for strict (any malicious = block).', 'vms-span-checker' ); ?></p>
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="vt_max_suspicious"><?php esc_html_e( 'Max suspicious engines', 'vms-span-checker' ); ?></label></th>
					<td>
						<?php
						...
						?>
						<input type="number" name="vt_max_suspicious" id="vt_max_suspicious" value="<?php echo esc_attr( $vt_susp_val ); ?>" min="0" max="100" class="small-text" placeholder="<?php esc_attr_e( 'off', 'vms-span-checker' ); ?>">
						<p class="description"><?php esc_html_e( 'Leave empty to ignore “suspicious” counts. If set, block when suspicious is greater than this value.', 'vms-span-checker' ); ?></p>
					</td>
				</tr>
			</table>
			<p class="submit">
				<input type="submit" class="button button-primary" value="<?php esc_attr_e( 'Save VirusTotal thresholds', 'vms-span-checker' ); ?>">
			</p>
		</form>
		<form method="post" class="wsc-admin__inline-form wsc-api-vt-add">
			<?php wp_nonce_field( 'wsc_api_action', 'wsc_api_nonce' ); ?>
			<input type="text" name="virustotal_key" placeholder="<?php esc_attr_e( 'Enter VirusTotal API key', 'vms-span-checker' ); ?>" class="regular-text" required>
			<input type="submit" class="button button-primary" value="<?php esc_attr_e( 'Add key', 'vms-span-checker' ); ?>">
		</form>
		...
		<?php if ( ! empty( $virustotal_config['keys'] ) ) : ?>
			<div class="wsc-admin__table-wrap wsc-api-vt-table">
				<table class="widefat striped">
					<thead>
					<tr>
						<th><?php esc_html_e( 'API key', 'vms-span-checker' ); ?></th>
						...
						<tr>
							...
							<td>
								<form method="post" class="wsc-inline-form">
									...
									<input type="submit" class="button button-secondary" value="<?php esc_attr_e( 'Delete', 'vms-span-checker' ); ?>">
								</form>
							</td>
						</tr>
					<?php endforeach; ?>
		...
		<?php else : ?>
			<p class="wsc-admin__empty-hint"><?php esc_html_e( 'No VirusTotal API keys have been added yet.', 'vms-span-checker' ); ?></p>
		<?php endif; ?>
	</div>
</div>
