...
<div class="wrap wsc-admin">
	...
	vms_span_checker_admin_page_header(
		...
		__( 'Add reCAPTCHA protection to WordPress login forms to prevent brute force attacks.', 'vms-span-checker' )
	);
	?>

	<?php if ( $updated ) : ?>
		<div class="updated"><p><?php esc_html_e( 'Login Guard settings saved.', 'vms-span-checker' ); ?></p></div>
	<?php endif; ?>
	...
	<div class="wsc-card" style="max-width: 800px;">
		<form method="post">
			...
			<h3><?php esc_html_e( 'General Settings', 'vms-span-checker' ); ?></h3>
			<table class="form-table" role="presentation">
				<tr>
					<th scope="row"><?php esc_html_e( 'Enable Login Guard', 'vms-span-checker' ); ?></th>
					<td>
						<?php
						vms_span_checker_admin_switch(
							array(
								...
								'label'       => __( 'Enable Login Guard protection', 'vms-span-checker' ),
								'description' => __( 'Adds security features to the WordPress login page.', 'vms-span-checker' ),
							)
						);
						?>
					</td>
					...
					<td>
						...
						vms_span_checker_admin_switch(
							array(
								...
								'label'       => __( 'Add Google reCAPTCHA to login form', 'vms-span-checker' ),
								'description' => __( 'Requires reCAPTCHA to be completed before login. Helps prevent automated login attempts.', 'vms-span-checker' ),
							)
						);
						?>
				...
				<tr>
					<th scope="row"><label for="login_guard_scope"><?php esc_html_e( 'Login Page', 'vms-span-checker' ); ?></label></th>
					<td>
						<select name="login_guard_scope" id="login_guard_scope">
							<option value="default" <?php selected( (string) ( $cfg['login_guard_scope'] ?? 'default' ), 'default' ); ?>><?php esc_html_e( 'Default WordPress login (wp-login.php)', 'vms-span-checker' ); ?></option>
							<option value="specific" <?php selected( (string) ( $cfg['login_guard_scope'] ?? 'default' ), 'specific' ); ?>><?php esc_html_e( 'Custom login page(s)', 'vms-span-checker' ); ?></option>
						</select>
						<p class="description"><?php esc_html_e( 'Choose where to apply Login Guard protection.', 'vms-span-checker' ); ?></p>
					</td>
				</tr>
				<tr id="login_guard_pages_row" style="display:none;">
					<th scope="row"><label><?php esc_html_e( 'Select Login Pages', 'vms-span-checker' ); ?></label></th>
					<td>
						<div class="wsc-page-selector">
							<div class="wsc-page-search-wrap">
								<input type="text" id="login_guard_page_search" class="regular-text" placeholder="<?php esc_attr_e( 'Search pages...', 'vms-span-checker' ); ?>" autocomplete="off">
						...
						<p class="description"><?php esc_html_e( 'Select pages that contain custom login forms (e.g., WooCommerce My Account, membership plugin login pages).', 'vms-span-checker' ); ?></p>
					</td>
				</tr>
				<tr id="login_guard_selector_row" style="display:none;">
					<th scope="row"><label for="login_guard_form_selector"><?php esc_html_e( 'Form Selector', 'vms-span-checker' ); ?> <span style="color:#d63638;">*</span></label></th>
					<td>
						...
						<p class="description">
							<?php esc_html_e( 'CSS selector to identify the login form. Examples: .woocommerce-form-login, #my-login-form', 'vms-span-checker' ); ?>
							<br>
							<strong><?php esc_html_e( 'Required for custom login pages to ensure reCAPTCHA is applied only to the correct form.', 'vms-span-checker' ); ?></strong>
						</p>
					</td>
				</tr>
			</table>
			...
			<p class="submit">
				<button type="submit" name="wsc_login_guard_save" value="1" class="button button-primary">
					<?php esc_html_e( 'Save Settings', 'vms-span-checker' ); ?>
				</button>
			</p>
		</form>
	</div>
	...
	<div class="wsc-card" style="max-width: 800px; margin-top: 20px;">
		<h3><?php esc_html_e( 'How It Works', 'vms-span-checker' ); ?></h3>
		<ol>
			<li><?php esc_html_e( 'Login Guard adds reCAPTCHA verification to the WordPress login page.', 'vms-span-checker' ); ?></li>
			<li><?php esc_html_e( 'Users must complete the reCAPTCHA before the login button becomes active (v2) or tokens are verified server-side (v3).', 'vms-span-checker' ); ?></li>
			<li><?php esc_html_e( 'This helps prevent brute force attacks and automated login attempts.', 'vms-span-checker' ); ?></li>
		</ol>
		<h4><?php esc_html_e( 'Recommended Setup', 'vms-span-checker' ); ?></h4>
		<ul style="list-style: disc; margin-left: 20px;">
			<li><strong><?php esc_html_e( 'reCAPTCHA v2', 'vms-span-checker' ); ?>:</strong> <?php esc_html_e( 'Shows a checkbox challenge. Login button is disabled until reCAPTCHA is completed.', 'vms-span-checker' ); ?></li>
			<li><strong><?php esc_html_e( 'reCAPTCHA v3', 'vms-span-checker' ); ?>:</strong> <?php esc_html_e( 'Invisible scoring system. No user interaction required, but suspicious logins may be blocked.', 'vms-span-checker' ); ?></li>
		</ul>
	</div>
</div>
